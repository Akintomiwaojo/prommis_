...
from prommis.util import scale_time_discretization_equations, copy_first_steady_state
...
class CocurrentSlurryLeachingFlowsheetData(FlowsheetBlockData):
    ...
    def _set_inputs(self):
        ...
        """
        # Liquid feed state
        self.leach.liquid_inlet.flow_vol.fix(0 * units.L / units.hour)
        self.leach.liquid_outlet.flow_vol.fix(0 * units.L / units.hour)
        ...
        # Solid feed state
        self.leach.solid_inlet.flow_mass.fix(0 * units.kg / units.hour)
        self.leach.solid_outlet.flow_mass.fix(0 * units.kg / units.hour)
...
def create_batch_leaching_model(n_tanks, time_set=range(25), time_units=units.hr):
    """
    Build and initialize a dynamic batch leaching model with UKY parameters.

    Requires tests/leaching_{n_tanks}_tank(s).json to exist as steady-state
    initial conditions (created by running leach_flowsheet.py).
    """
    if n_tanks == 1:
        tank_str = "tank"
    else:
        tank_str = "tanks"
    print(f"Creating batch leaching model with {n_tanks} {tank_str}")
    ...
    # Load steady-state initial conditions (from leach_flowsheet.py)
    ic_path = os.path.join("tests", f"leaching_{n_tanks}_{tank_str}.json")
    from_json(m, fname=ic_path, wts=StoreSpec.value())

    m.fs.reduce_dae_index()
    m.fs.scale_dynamics(1 / (time_set[-1] - time_set[0]))
    m.fs.fix_initial_conditions()
    copy_first_steady_state(m, m.fs.time)

    # Initialize any remaining None vars (e.g. accumulation terms)
    for var in m.component_data_objects(ctype=Var, descend_into=True):
        if var.value is None:
            var.set_value(0)

    # Batch: no inflow or outflow at any time point
    m.fs.leach.liquid_inlet.flow_vol.fix(0 * units.L / units.hour)
    m.fs.leach.solid_inlet.flow_mass.fix(0 * units.kg / units.hour)

    # outlet_flow_rate_eqn becomes 0==0 with zero flows — deactivate to
    # avoid a rank-deficient Jacobian (volume_frac_stream is still
    # constrained by the holdup equations)
    for t in m.fs.time:
        for s in m.fs.leach.mscontactor.elements:
            m.fs.leach.mscontactor.outlet_flow_rate_eqn[t, s].deactivate()
...
if __name__ == "__main__":
    m = create_batch_leaching_model(n_tanks=1)
