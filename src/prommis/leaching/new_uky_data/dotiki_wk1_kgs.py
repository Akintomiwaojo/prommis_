...
m.fs.leach.liquid_inlet.conc_mass_comp[0, "H"].fix(2 * 0.5 * 1e3 * units.mg / units.L)
m.fs.leach.liquid_inlet.conc_mass_comp[0, "SO4"].fix(0.5 * 96e3 * units.mg / units.L)
...
absent_solid = ["Ce2O3", "Pr2O3", "Gd2O3"]
...
# Initialize ONCE, after the scaling factors are set (the scaling is what lets
# the initializer converge at this aggressive acid loading). Do NOT add an
# earlier cold init: at this operating point it fails, and a failed init can
# leave the model with DoF != 0, which then trips the precheck on this call
# ("Degrees of freedom for fs.leach were not equal to 0").
...
for e in m.fs.coal.component_list:
    if e not in ["Ce2O3", "Pr2O3", "Gd2O3", "inerts"]:
